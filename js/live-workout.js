...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!dayId) {
        workoutContainer.innerHTML = '<h1 class="text-3xl text-red-500">No workout day specified.</h1>';
        return;
    }
...
async function fetchExercises(dayId) {
    try {
        ...
        if (exercises.length > 0) {
            ...
            workoutContainer.innerHTML = '<h1 class="text-3xl">This workout day has no exercises.</h1>';
        }

    } catch (error) {
        console.error('Error fetching exercises:', error);
        workoutContainer.innerHTML = '<h1 class="text-3xl text-red-500">Could not load workout.</h1>';
    }
}
...
function displayExercise(index) {
    ...
    setsRepsEl.textContent = `Sets: ${exerciseData.sets} | Reps: ${exerciseData.reps}`;
...
function finishWorkout() {
    workoutContainer.innerHTML = `
        <h1 class="text-6xl font-bold text-green-400">Workout Complete!</h1>
        <p class="text-2xl mt-4">Great job!</p>
        <a href="workoutclient.html" class="mt-8 inline-block bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-6 rounded">Back to Program</a>
    `;
}
