...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (currentPlanId) {
        pageTitle.textContent = 'Edit Meal Plan';
        // await loadMealPlanForEditing(currentPlanId); // To be implemented
    } else {
        pageTitle.textContent = 'Create New Meal Plan';
    }
...
async function loadMealPlanForEditing(planId) {
    try {
        mealDaysContainer.innerHTML = '<p>Loading meal plan data...</p>';

        // 1. Fetch the main plan details
        ...
        // 4. Fetch all food items for all days
        ...
        mealDaysContainer.innerHTML = ''; // Clear loading message

        // 5. Populate the days and foods
        days.forEach(day => {
        ...
        mealDaysContainer.innerHTML = `<p class="text-red-500">Failed to load meal plan data: ${error.message}</p>`;
    }
}
...
function addDayBlockWithData(day, foods) {
    ...
    dayBlock.innerHTML = `
        <div class="flex justify-between items-center mb-2">
            <input type="text" value="${day.day_name}" class="day-name-input text-lg font-bold border-b-2" placeholder="Day Name">
        ...
        <div class="meals-container space-y-3">
            ${createMealSection('Breakfast')}
            ${createMealSection('Lunch')}
            ${createMealSection('Dinner')}
            ${createMealSection('Snacks')}
        </div>
    `;
    mealDaysContainer.appendChild(dayBlock);

    // Populate foods into their respective meal sections
...
function addFoodToMealWithData(container, foodData) {
    ...
    foodElement.innerHTML = `
        ...
        <input type="text" class="food-quantity w-32 border rounded px-1" placeholder="Quantity (e.g., 100g)" value="${foodData.quantity || ''}">
...
function addDayBlock(dayData = null) {
    ...
    dayBlock.innerHTML = `
        <div class="flex justify-between items-center mb-2">
            <input type="text" value="${dayData ? dayData.name : `Day ${dayCounter}`}" class="day-name-input text-lg font-bold border-b-2" placeholder="Day Name">
            <button type="button" class="remove-day-btn bg-red-200 hover:bg-red-300 text-red-800 font-bold py-1 px-2 rounded text-sm">Remove Day</button>
        </div>
        <div class="meals-container space-y-3">
             <!-- Pre-defined meals -->
...
function createMealSection(mealType) {
    return `
        <div class="meal-section border-t pt-2" data-meal-type="${mealType}">
            <div class="flex justify-between items-center">
                 <h4 class="font-semibold">${mealType}</h4>
                 <button type="button" class="add-food-btn text-blue-500 hover:text-blue-700 text-sm font-bold">Add Food +</button>
            </div>
            <div class="foods-container space-y-1 mt-1">
                <!-- Food items for this meal will be added here -->
            </div>
        </div>
    `;
}
...
async function handleFoodSearch(e) {
    ...
    try {
        ...
        if (foods.length === 0) {
            foodSearchResultsContainer.innerHTML = '<p>No foods found.</p>';
        } else {
            foods.forEach(food => {
        ...
        foodSearchResultsContainer.innerHTML = '<p class="text-red-500">Error searching.</p>';
    }
}
...
function addFoodToMeal(food) {
    ...
    foodElement.innerHTML = `
        ...
        <input type="text" class="food-quantity w-32 border rounded px-1" placeholder="Quantity (e.g., 100g)">
        <button type="button" class="remove-food-btn text-red-500 hover:text-red-700">✖</button>
    `;
