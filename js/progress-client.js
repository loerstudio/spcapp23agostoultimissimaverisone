...
async function loadPageData() {
    try {
        ...
        if (currentGoal) {
            ...
            goalDisplay.innerHTML = '<p>You have not set a goal yet. Fill out the form below to get started!</p>';
        ...
        goalSection.innerHTML = '<p class="text-red-500">Could not load your goal information.</p>';
    }
}

function displayGoalInfo() {
    goalDisplay.innerHTML = `
        <p><strong>Description:</strong> ${currentGoal.description}</p>
        <p><strong>Start Date:</strong> ${new Date(currentGoal.start_date).toLocaleDateString()}</p>
        <p><strong>End Date:</strong> ${new Date(currentGoal.end_date).toLocaleDateString()}</p>
        <button id="edit-goal-btn" class="mt-2 bg-yellow-500 text-white py-1 px-3 rounded">Edit Goal</button>
    `;
...
async function loadProgressPhotos(goalId) {
    try {
        ...
        if (photos.length === 0) {
            progressGallery.innerHTML = '<p>No photos uploaded for this goal yet.</p>';
            return;
        }
        ...
        progressGallery.innerHTML = '<p class="text-red-500">Could not load photos.</p>';
    }
}
