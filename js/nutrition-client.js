    ...
    try {
        ...
        if (planError) {
            if (planError.code === 'PGRST116') {
                planNameEl.textContent = 'No Meal Plan Assigned';
                planDescriptionEl.textContent = 'Your trainer has not assigned a meal plan to you yet.';
            } else {
                throw planError;
            }
            return;
        ...
        planNameEl.textContent = 'Error';
        planDescriptionEl.textContent = 'Could not load your meal plan.';
    }
}
...
function displayMealPlan(plan, days, dayFoods) {
    ...
    if (days.length === 0) {
        daysAccordion.innerHTML = '<p>This meal plan has no days defined yet.</p>';
        return;
    }
    ...
    days.forEach(day => {
        ...
        const meals = foodsForDay.reduce((acc, food) => {
            const type = food.meal_type || 'Uncategorized';
            if (!acc[type]) {
                acc[type] = [];
            }
            acc[type].push(food);
