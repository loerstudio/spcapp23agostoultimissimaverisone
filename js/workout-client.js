    ...
    try {
        ...
        if (programError) {
            if (programError.code === 'PGRST116') {
                programNameEl.textContent = 'No Program Assigned';
                programDescriptionEl.textContent = 'Your trainer has not assigned a workout program to you yet.';
            } else {
                throw programError;
            }
            return;
        ...
        programNameEl.textContent = 'Error';
        programDescriptionEl.textContent = 'Could not load your workout program.';
    }
}
...
function displayProgram(program, days, dayExercises) {
    ...
    if (days.length === 0) {
        daysAccordion.innerHTML = '<p>This program has no workout days defined yet.</p>';
        return;
    }
                        ...
                        <div class="exercise-card p-4 border rounded-lg">
                            ...
                            <div class="flex flex-col md:flex-row gap-4 mt-2">
                                ...
                                <div class="flex-grow">
                                    ...
                                    ${ex.notes ? `<p class="mt-1 text-sm text-gray-600"><strong>Notes:</strong> ${ex.notes}</p>` : ''}
                                    ${ex.exercises.video_url ? `<a href="${ex.exercises.video_url}" target="_blank" class="text-blue-500 hover:underline mt-2 inline-block">Watch Video</a>` : ''}
                                </div>
                            </div>
                        </div>
