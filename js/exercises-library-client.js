...
async function fetchAndDisplayExercises() {
    ...
    try {
        ...
        exerciseListContainer.innerHTML = '<p class="text-red-500">Could not load exercise library.</p>';
    }
}
...
function displayExercises(exercises) {
    ...
    if (exercises.length === 0) {
        exerciseListContainer.innerHTML = '<p>No exercises found in the library.</p>';
        return;
    }
    ...
    exercises.forEach(exercise => {
        ...
        card.innerHTML = `
            ...
            ${exercise.video_url ? `<a href="${exercise.video_url}" target="_blank" class="text-blue-500 hover:underline mt-4 self-start">Watch Video</a>` : ''}
        `;
