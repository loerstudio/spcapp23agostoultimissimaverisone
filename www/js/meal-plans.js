...
const fetchAndDisplayMealPlans = async () => {
    try {
        ...
        mealPlanListContainer.innerHTML = `<p class="text-red-500">Errore nel caricamento dei piani alimentari: ${error.message}</p>`;
    }
};
...
const displayMealPlans = (mealPlans) => {
    ...
    if (!mealPlans || mealPlans.length === 0) {
        mealPlanListContainer.innerHTML = '<p>Non hai ancora creato nessun piano alimentare.</p>';
        return;
    }
    ...
    mealPlans.forEach(plan => {
        ...
        const clientName = plan.profiles ? plan.profiles.full_name : 'Non assegnato';
        ...
        planCard.innerHTML = `
            ...
            <div class="flex justify-end space-x-2">
                <a href="edit-meal-plan.html?id=${plan.id}" class="edit-plan-btn bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">Modifica</a>
                <button data-plan-id="${plan.id}" class="delete-plan-btn bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded">Elimina</button>
            </div>
        `;
...
const deleteMealPlan = async (planId) => {
    try {
        ...
        console.error('Errore durante l\'eliminazione del piano alimentare:', error);
        alert(`Impossibile eliminare il piano alimentare: ${error.message}`);
    }
};
