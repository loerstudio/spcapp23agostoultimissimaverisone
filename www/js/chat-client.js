    ...
    try {
        ...
        if (clientError) {
            if (clientError.code === 'PGRST116') {
                messagesContainer.innerHTML = '<p class="text-center text-gray-500">Non sei assegnato a un allenatore.</p>';
            } else {
                throw clientError;
            }
            return;
        ...
        chatWithName.textContent = `Chat con ${trainerProfile.full_name}`;
        ...
        messagesContainer.innerHTML = '<p class="text-red-500">Impossibile caricare la chat.</p>';
    }
}
...
async function loadMessageHistory(trainerId) {
    try {
        ...
        messagesContainer.innerHTML = '<p>Impossibile caricare la cronologia messaggi.</p>';
    }
}
