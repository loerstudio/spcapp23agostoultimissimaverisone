...
const fetchAndDisplayPrograms = async () => {
    try {
        ...
        programListContainer.innerHTML = `<p class="text-red-500">Errore nel caricamento dei programmi: ${error.message}</p>`;
    }
};
...
const displayPrograms = (programs) => {
    ...
    if (!programs || programs.length === 0) {
        programListContainer.innerHTML = '<p>Non hai ancora creato nessun programma.</p>';
        return;
    }
    ...
    programs.forEach(program => {
        ...
        const clientName = program.profiles ? program.profiles.full_name : 'Non assegnato';

        programCard.innerHTML = `
            <h3 class="text-xl font-bold mb-2">${program.name}</h3>
            <p class="text-gray-600 mb-2">Assegnato a: ${clientName}</p>
            <p class="text-gray-700 mb-4">${program.description || ''}</p>
            <div class="flex justify-end space-x-2">
                <a href="edit-program.html?id=${program.id}" class="edit-program-btn bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">Modifica</a>
                <button data-program-id="${program.id}" class="delete-program-btn bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded">Elimina</button>
            </div>
        `;
    ...
    document.querySelectorAll('.delete-program-btn').forEach(button => {
        button.addEventListener('click', async (e) => {
            ...
            if (confirm('Sei sicuro di voler eliminare questo programma? Questa azione non può essere annullata.')) {
...
const deleteProgram = async (programId) => {
    try {
        ...
        alert(`Impossibile eliminare il programma: ${error.message}`);
    }
};
